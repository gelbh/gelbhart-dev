...
export class ItemManager {
  ...
  generateDots() {
    ...
    const existingDots =
      this.controller.element.querySelectorAll(".pacman-dot");
    ...
    for (let x = margin; x < viewportWidth - margin; x += dotSpacing) {
      for (let y = minY; y < maxY; y += dotSpacing) {
        ...
        const inBufferZone = sectionZones.some((zone) => {
          ...
          return (
            (zone.locked && y >= zone.top && y <= zone.bottom) ||
            (y >= zone.bottom - 80 && y <= zone.bottom)
          ); // Bottom buffer of all sections
        });

        // Check if this position was already collected
        const posKey = `${Math.round(x)},${Math.round(y)}`;
        const alreadyCollected =
          this.controller.collectedDotPositions.has(posKey);
    ...
    for (let i = 0; i < sections; i++) {
      ...
      pelletPositions.forEach((pos) => {
        if (pos.y < maxY && pos.y > minY) {
          ...
          const inBufferZone = sectionZones.some((zone) => {
            return (
              (zone.locked && pos.y >= zone.top && pos.y <= zone.bottom) ||
  ...
  createItem(x, y, type) {
    ...
    item.className = `pacman-item ${
      itemConfig.positive ? "item-positive" : "item-negative"
    }`;
    item.innerHTML = `
  ...
  optimizeDotVisibility() {
    ...
    this.controller.dots.forEach((dot) => {
      ...
      const inRenderRange =
        dot.y >= viewportTop - renderBuffer &&
  ...
  checkItemCollisions() {
    ...
    this.controller.items.forEach((item) => {
      ...
      if (distance < collisionRadius) {
        ...
        const pointsEarned =
          item.config.points *
          (this.controller.activeEffects.doublePoints ? 2 : 1);
        this.controller.score += pointsEarned;
        this.controller.updateHUD();
    ...
    this.controller.powerModeTimer = setTimeout(() => {
      ...
      this.controller.ghosts.forEach((ghost) => {
        if (!ghost.eaten) {
          ...
          const sprite = ghost.element.querySelector(".ghost-sprite");
          if (sprite) {
            sprite.src = this.controller.getGhostSprite(
              ghost.color,
              ghost.direction,
  ...
  removeEffectCooldown(effectName) {
    const existingBar = this.controller.pacmanTarget.querySelector(
