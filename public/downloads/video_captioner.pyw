#!/usr/bin/env python3
"""Video Captioner GUI launcher (.pyw variant).
...
def _show_launch_error(error):
    try:
        ...
        messagebox.showerror(
            "Video Captioner failed to start",
            "Unable to launch Video Captioner.\n\n"
            f"Error: {error}\n\n"
            "First run requires internet access to fetch support files.\n"
            "Please check your connection, then open the GUI script again.\n"
            "If the issue persists, re-download the GUI script and retry.",
        )
        root.destroy()
    except Exception:
        pass
...
def _show_bootstrap_notice():
    try:
        import tkinter as tk

        root = tk.Tk()
        root.title("Video Captioner")
        root.geometry("520x160")
        root.resizable(False, False)
        label = tk.Label(
            root,
            text="Preparing Video Captioner for first run...\n"
            "Downloading support files and dependencies if needed.\n"
...
if __name__ == "__main__":
    bootstrap_notice = _show_bootstrap_notice()
    try:
        _bootstrap_core_if_missing()
        _ensure_customtkinter()
        if bootstrap_notice is not None:
            bootstrap_notice.destroy()
